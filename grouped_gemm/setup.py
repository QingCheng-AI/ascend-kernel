    ...
    def build_extension(self, ext):
        ...
        if ext.name == "grouped_gemm":
            try:
                ...
                subprocess.run(
                    ["cp", "-r", "op_kernel", f"{tmp_dir}/grouped_gemm_autogen"],
                    check=True,
                )
                cur_dir = os.getcwd()
                os.chdir(f"{tmp_dir}/grouped_gemm_autogen")
                # 执行 build.sh
...
grouped_gemm = CppExtension(
    "grouped_gemm",  # 模块名称（需与 PYBIND11_MODULE 中一致）
    sources=[
        "./src/pybind.cpp",
        "./src/grouped_gemm.cpp",
    ],  # 包含绑定代码的 C++ 文件
    include_dirs=[
        pybind11_include,
