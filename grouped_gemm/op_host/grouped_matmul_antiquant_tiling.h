...
TILING_DATA_FIELD_DEF(
    uint32_t, singleN); // If sequential write， the value should be zero!
TILING_DATA_FIELD_DEF(uint32_t,
                      quantParam); // in quant case, PerToken: 1; in antiquant
                                   // case, the value represents PerGroupSize
...
BEGIN_TILING_DATA_DEF(GMMAntiquantTilingData)
TILING_DATA_FIELD_DEF_STRUCT(GMMBaseParams, gmmBaseParams);
TILING_DATA_FIELD_DEF_STRUCT(TCubeTiling, mmTilingData);
END_TILING_DATA_DEF;

REGISTER_TILING_DATA_CLASS(GroupedMatmulAntiquant, GMMAntiquantTilingData)
