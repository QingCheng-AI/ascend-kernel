...
void GroupedGEMV(at::Tensor x, at::Tensor weight,
                   at::Tensor scale, at::Tensor groupList, 
                   GroupedGemmType type, at::Tensor output) {
    auto acl_stream = c10_npu::getCurrentNPUStream().stream(true);
    aclTensor *x_ = createTensor(x, 0);
    aclTensor *weight_ = type == GroupedGemmType::FP4 ? createTensor(weight, 1)
                                                      : createTensor(weight, 0);
    aclTensor *scale_ =
        createTensor(scale, 0);
    aclTensor *groupList_ = createTensor(groupList, 0);
    aclTensor *output_ = createTensor(output, 0);
    uint64_t workspaceSize = 0;
    void *workspaceAddr = nullptr;
    aclOpExecutor *executor;
    auto ret = aclnnGroupedSoftGemvGetWorkspaceSize(x_, weight_, scale_, \
                                                    groupList_, output_, &workspaceSize, &executor);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnGroupedSoftGemvGetWorkspaceSize failed. ERROR: %d\n", ret));
    // 根据第一段接口计算出的workspaceSize申请device内存
    workspaceAddr = WorkspaceManager().getFromMap(workspaceSize);
    // std::chrono::time_point<std::chrono::system_clock> start, end;
    // start = std::chrono::system_clock::now();
    ret = aclnnGroupedSoftGemv(workspaceAddr, workspaceSize, executor, acl_stream);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnGroupedSoftGemv failed. ERROR: %d\n", ret));
    // ret = aclrtSynchronizeStream(acl_stream);
    // end = std::chrono::system_clock::now();
    // std::chrono::duration<double> elapsed_seconds = end - start;
    // std::time_t end_time = std::chrono::system_clock::to_time_t(end);
