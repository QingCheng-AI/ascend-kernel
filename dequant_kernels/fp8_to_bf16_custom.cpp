...
class KernelFp8ToFloat {
  public:
    __aicore__ inline KernelFp8ToFloat() {}
    __aicore__ inline ~KernelFp8ToFloat() { inQueueY.FreeTensor(this->yLocal); }
    __aicore__ inline void Init(GM_ADDR x, GM_ADDR y, GM_ADDR z, uint32_t M,
        ...
        uint32_t m_start = AscendC::GetBlockIdx() * (this->sub_m * this->sub_n);
        uint32_t tail_m = M - AscendC::GetBlockIdx() * this->sub_m;
        ...
        yGm.SetGlobalBuffer((__gm__ float *)y, scale_size_64);
  ...
  private:
    ...
    __aicore__ inline void Compute(int32_t progress) {
        ...
        // 【step4: 将结果乘以 scale】
        int32_t index = (AscendC::GetBlockIdx() +
                         progress / (this->BLOCK_SIZE * this->scale_n)) *
                            this->scale_n +
...
fp8_to_bf16_custom(GM_ADDR x, GM_ADDR scale, GM_ADDR z, uint32_t M, uint32_t N,
                   uint32_t BLOCK_SIZE) {
    KernelFp8ToFloat op;
